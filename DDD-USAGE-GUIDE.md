...
An append-only log of every spec change across the project. Written by ddd-tool on save, `/ddd-update` after applying changes, `/ddd-create` after generating specs, and `/ddd-sync` when it detects new-spec-logic drift. Read by `/ddd-implement` (no flags) to determine what to implement next.
...
- id: "chg-0001"
  ...
  source: ddd-tool          # ddd-tool | ddd-update | ddd-create | ddd-sync
  scope:
...
| Source | When written |
|--------|-------------|
| `ddd-tool` | Every canvas save that modifies a spec file |
| `ddd-update` | After applying spec changes from `/ddd-update` |
| `ddd-create` | After all specs are generated by `/ddd-create` |
| `ddd-sync` | When drift analysis finds `new_spec_logic` — spec has changes code doesn't implement |
