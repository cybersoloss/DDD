...
my-project/
  ...
  .ddd/
    ...
    change-history.yaml          # Append-only change log — written by ddd-tool, /ddd-update, /ddd-create, /ddd-sync
...
cross_cutting_patterns:
  ...
  # See Section 12.4 for full schema and examples.
  # Example:
  #   soft_delete:
...
| `component` | string | Component type (e.g., "stat-card", "item-list", "card-grid", "detail-card", "button-group", "page-header", "status-bar", "chat-interface", "markdown-viewer", "tree-view") or shared component ID |
...
| `streaming_behavior` | object? | How the section displays progressively streamed content (e.g., AI agent token-by-token output via SSE): `{ source: 'sse' \| 'websocket', target_field: string, append: boolean, typing_indicator?: boolean }`. Use with `chat-interface` or any component that displays AI-generated text. `/ddd-implement` generates the SSE/WebSocket listener that appends tokens to the target field in real time. |
...
| `streaming` | object? | Progressive token output for real-time UIs: `{ enabled: boolean, format: 'sse' \| 'websocket', chunk_field?: string }`. When enabled, `/ddd-implement` streams agent responses token-by-token instead of waiting for the full response. Pair with `chat-interface` or `streaming_behavior` on the UI side. |
...
#### guardrail
Inline validation node. Guardrails are **inline and sequential** — data flows through them in order. They are NOT sidecars or parallel watchers. Works in both **agent flows** (before/after agent_loop) and **traditional flows** (for business rule validation, schema checks, input sanitization). In traditional flows, use guardrail instead of a process node when you need structured, rule-based validation with explicit pass/block semantics.

| Spec Field | Type | Values |
|------------|------|--------|
| `position` | string | `'input' \| 'output'` |
| `checks` | GuardrailCheck[] | `{ type: string, action: 'block' \| 'warn' \| 'log', rule?: string }`. Built-in types: `content_policy`, `prompt_injection`, `file_type`, `file_size`, `required_fields`, `business_rule`. For `business_rule` type, use `rule` field to describe the validation logic (e.g., `"Every Goal must have at least one Objective"`). |
...
| `--fix-drift` | Resolve all drift using the decision tree: metadata→hash, code-ahead→reflect, new-logic→implement |
...
**What it does:**
1. Reads `ddd-project.json`, all domain.yaml files, `.ddd/mapping.yaml`, `.ddd/change-history.yaml`, `specs/schemas/*.yaml`, `specs/ui/pages.yaml`, `specs/ui/*.yaml`, `specs/infrastructure.yaml`, and `specs/architecture.yaml`
...
4. Checks scaffold state per pillar (package.json, entry point, database schema, page components, infrastructure configs)
5. Displays tables per pillar — Logic (flows), Interface (pages), Data (schemas), Infrastructure (services) — with status and implementation dates
6. Shows pending `change-history` entries if any exist
7. Suggests next actions using **safe recommendations** — never recommends `/ddd-implement` for drifted flows without confirming the drift type is "new logic". See Section 12.1.
...
2. Critically evaluates each shortfall through 7 filters:
   ...
   - **Pillar balance** — disproportionate gap counts or coverage issues across pillars?
...
3. Classifies findings into pattern categories per pillar — Logic: stealth_http, encryption, soft_delete, etc.; Interface: component_composition, data_fetching, etc.; Data: index_optimization, migration_pattern, etc.; Infrastructure: docker_config, startup_orchestration, etc. (see Section 12.3 for full list) — using `architecture.yaml` cross_cutting_patterns as reference
...
`/ddd-evolve` deduplicates across projects, evaluates each shortfall through 7 filters (already possible? recurring? specific? breaking? adequate workaround? intentional? pillar balance?), and classifies each as:
...
Hash mismatch detected
  ...
  │   └─ YES → /ddd-reflect {domain/flow} to capture patterns,
  │            then /ddd-promote --review, then /ddd-sync
  │
...
3. **Never update a hash when code is ahead of spec.** If implementation has details the spec doesn't describe, the correct action is `/ddd-reflect` to capture patterns as annotations, then `/ddd-promote --review` to enrich the spec, then `/ddd-sync` to update the hash. Updating the hash directly means future `/ddd-implement` runs will generate code WITHOUT those details.
...
### 12.2 change-history.yaml

**Path:** `.ddd/change-history.yaml`

An append-only log of every spec change across the project. Written by ddd-tool on save, `/ddd-update` after applying changes, `/ddd-create` after generating specs, `/ddd-sync` when it detects new-spec-logic drift, `/ddd-promote` after enriching specs with promoted patterns, and `/ddd-reverse` after generating specs from code. Read by `/ddd-implement` (no flags) to determine what to implement next.

**Never delete entries.** Only update `status`, `implemented_at`, and `code_files` after implementation.

```yaml
- id: "chg-0001"
  timestamp: "2026-02-22T10:14:33Z"
  source: ddd-tool          # ddd-tool | ddd-update | ddd-create | ddd-sync | ddd-promote | ddd-reverse
  change_type: modified      # added | modified | removed
  scope:
    level: L3               # L1 = domain config | L2 = domain map | L3 = flow
    domain: auth
    flow: user-login
    pillar: logic           # logic | data | interface | infrastructure | null
  spec_file: specs/domains/auth/flows/user-login.yaml
  spec_checksum: "a3f9c1d2e4b5"   # SHA-256, first 12 chars
  status: pending_implement       # pending_implement | implemented
  implemented_at: null
  code_files: []

- id: "chg-0002"
  timestamp: "2026-02-22T10:22:14Z"
  source: ddd-tool
  scope:
    level: L3
    domain: auth
    flow: user-register
    pillar: logic
  spec_file: specs/domains/auth/flows/user-register.yaml
  spec_checksum: "b7d2e4f1c8a9"
  status: implemented
  implemented_at: "2026-02-22T10:35:00Z"
  code_files:
    - src/routes/auth.ts
    - src/services/user-service.ts
    - tests/routes/auth.test.ts
...
| Source | When written | Typical `change_type` |
|--------|-------------|----------------------|
| `ddd-tool` | Every canvas save that modifies a spec file | `modified` |
| `ddd-update` | After applying spec changes from `/ddd-update` | `added` or `modified` |
| `ddd-create` | After all specs are generated by `/ddd-create` | `added` |
| `ddd-sync` | When drift analysis finds `new_spec_logic` — spec has changes code doesn't implement | `modified` |
| `ddd-promote` | After enriching specs with promoted annotation patterns | `modified` |
| `ddd-reverse` | After generating specs from existing code | `added` |

### Status Lifecycle

```
pending_implement  →  (run /ddd-implement)  →  implemented
```
...
| Command | Behaviour |
|---------|-----------|
| `/ddd-implement` (no flags) | Reads pending entries → implements only those → marks implemented |
| `/ddd-implement --all` | Ignores change-history — implements everything |
| `/ddd-implement --flow x` | Ignores change-history — implements specific scope |
| `/ddd-implement --ignore-history` | Skips change-history → shows interactive list |
| `/ddd-sync` | Writes pending entries for new-spec-logic drift |
| `/ddd-test` (no flags) | Reads recently implemented entries → scoped test run |
| `/ddd-status` | Shows pending entries section if any exist |

### Deduplication Rules

- Same `spec_file` + same `spec_checksum` + existing `pending_implement` entry → skip, no duplicate
- Same `spec_file` + different checksum → new entry (spec changed again since last write)
- `status: implemented` entries are never overwritten — a new pending entry is added if the spec changes again after implementation
...
| **Code → Spec** | Does the spec describe everything the code does? | Need `/ddd-reflect` → `/ddd-promote` first |

Only update the hash when BOTH answers are YES.

### 12.3 Annotations Schema
...
### 12.4 Cross-Cutting Patterns in architecture.yaml
